...
router.get("/getalltickets", async (req, res) => {
  try {
    ...
    const processedTickets = tickets.map(ticket => ({
      ...
      headshotUrl: ticket.headshotUrl ? `${process.env.BASE_URL}/${ticket.headshotUrl}` : null,
      paymentProofUrl: ticket.paymentProofUrl ? `${process.env.BASE_URL}/${ticket.paymentProofUrl}` : null,
      attendees: ticket.attendees?.map(att => ({
        name: att.name,
        email: att.email,
        headshotUrl: att.headshotUrl ? `${process.env.BASE_URL}/${att.headshotUrl}` : null,
      })) || [],
