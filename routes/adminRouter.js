...
router.get("/getalltickets", async (req, res) => {
  try {
    ...
    const processedTickets = tickets.map(ticket => ({
      ...
      headshotUrl: ticket.headshotUrl ? new URL(ticket.headshotUrl, process.env.BASE_URL).href : null,
      paymentProofUrl: ticket.paymentProofUrl ? new URL(ticket.paymentProofUrl, process.env.BASE_URL).href : null,
      attendees: ticket.attendees?.map(att => ({
        name: att.name,
        email: att.email,
        headshotUrl: att.headshotUrl ? new URL(att.headshotUrl, process.env.BASE_URL).href : null,
      })) || [],
